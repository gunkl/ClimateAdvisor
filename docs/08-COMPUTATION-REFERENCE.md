...
| Can a manually/RF-remote-overridden whole-house fan keep running after all doors/windows close? | No — `_enforce_sealed_house_whf_guard()` is called from the event-driven sensor-closed handler (~1-3s), the 5-minute thermostatic backstop, and the restart RF-timer re-arm branch, so no WHF-activation path (CA-owned or override) can leave the fan running against a sealed house (Issue #882). | [§9e-H Sealed-House Guard](08-COMPUTATION-REFERENCE.md#h-sealed-house-guard--override-sessions-are-not-exempt-issue-882) |
...
| Why did the whole-house fan run 2.5 hours past when it should have paused, dropping the bedroom below the overnight floor? | A pause decision got deferred by the 300s fan-toggle rate limiter, but the code clearing `_natural_vent_active`/`_nat_vent_soft_start` ran unconditionally regardless of whether the deactivate command actually reached the fan — once cleared, `nat_vent_temperature_check()`'s own `if not self._natural_vent_active: return` guard meant nothing ever retried the stop. Fixed by a single sole-writer method, `_end_nat_vent_session()`, that only clears the flags once the paired `FanCommandResult` confirms the command ran (Issue #931). | [Exit Hierarchy — `_end_nat_vent_session()` sole writer](08-COMPUTATION-REFERENCE.md#exit-hierarchy) |
| Does the economizer have the same rate-limit/override gaps as nat-vent's `_end_nat_vent_session()` (#931) and `_apply_nat_vent_fsm_state_after_activation()` (#935)? | Yes — found independently while investigating the economizer specifically (Issue #936) and fixed with the direct mirrors `_deactivate_economizer()` (gated flag-clear) and `_apply_economizer_fsm_state_after_activation()` (gated post-await FSM-state write), same fallback-state reasoning per side. | [§17 — Mirrored onto the economizer (Issue #936)](08-COMPUTATION-REFERENCE.md#mirrored-onto-the-economizer-issue-936) · [`economizer-lifecycle-spec.md`](economizer-lifecycle-spec.md#rate-limit--override-symmetry-issue-936-mirrors-931935) |
...
| `vent_window_decay` | Window open, fan/WHF off, HVAC off | k_vent_window | 20 |
| `vent_fan_decay` | Window open, fan/WHF on, HVAC off | k_vent_fan | 20 |
| `solar_gain` | HVAC off, fan off, windows closed, T_in > T_out, daytime | k_solar | 20 |

> **Issue #587** retired the old `fan_only_decay` type (fan on, windows *closed* →
> `k_vent`) and split the old `ventilated_decay` type into `vent_window_decay` (window
> open, fan off) and `vent_fan_decay` (window open AND fan/WHF on). The bare `k_vent`
> field no longer exists. Both new types share `THERMAL_VENT_MIN_SAMPLES = 20` and
> `THERMAL_VENT_MIN_SIGNAL_F = 0.3` (`const.py`), driven by a single shared
> `_VENT_SPLIT_TYPES` table in `coordinator.py` rather than parallel per-type logic.
...
where `k_vent_eff = k_vent_window` when a window is open, `solar_factor` = sinusoidal 0→1→0
over daylight hours (8–18 local), `Q_hvac = ±k_active` when HVAC is driving toward setpoint.
`k_vent_fan` is learned and displayed (`get_engine_status()`/`get_thermal_model()`) but is
**not** wired into this ODE — a deliberate Issue #587 scope boundary, not an oversight; see
the Engine Visibility section of `docs/thermal-model-v3-spec.md`.
...
| `vent_window_decay` | 5 min | `THERMAL_PASSIVE_SAMPLE_INTERVAL_S` |
| `vent_fan_decay` | 5 min | `THERMAL_PASSIVE_SAMPLE_INTERVAL_S` |
...
`THERMAL_FAN_SAMPLE_INTERVAL_S` (the old 2-minute gate for `fan_only_decay`) was retired
in Issue #587 — both vent-split types now use the same 5-minute gate as `passive_decay`.
...
3. For `vent_window_decay` and `vent_fan_decay`: the ΔT guard is skipped
...
`vent_window_decay` and `vent_fan_decay` abandon after `THERMAL_DECAY_MAX_WINDOW_MINUTES
...
#### 5e-iv. `_update_thermal_model_cache()` — E6 Parameter Routing Fix (Issue #122; obs types renamed in #587)

Each committed observation updates the EWMA cache via `learning._update_thermal_model_cache()`.
The observation type determines which cache field is updated:

| Obs type | Updates cache field | Count field |
|---|---|---|
| `hvac_heat` | `k_active_heat`, `k_passive` | `observation_count_heat` |
| `hvac_cool` | `k_active_cool`, `k_passive` | `observation_count_cool` |
| `passive_decay` | `k_passive` only | `observation_count_passive` |
| `vent_window_decay` | `k_vent_window`; also `k_solar` when 2-param OLS fires (see §5e-v) | `observation_count_vent_window` |
| `vent_fan_decay` | `k_vent_fan` | `observation_count_vent_fan` |
| `solar_gain` | `k_solar` (from obs `k_solar` field) | `observation_count_solar` |

**E6 fix** (historical, Issue #122): Before Issue #122, the passive-observation branch
incorrectly wrote its rate to the ventilation parameter (`cache["k_vent"]`, the field
Issue #587 later retired). The fix removed that line — passive observations no longer
contaminate the ventilation parameter. Only ventilation-typed observations update their
own `k_vent_window`/`k_vent_fan` field.

#### 5e-v. Adaptive 2-Param Ventilated OLS (Issue #126; `vent_window_decay` only)

`vent_window_decay` observations optionally upgrade from 1-parameter OLS (solving only
...
`vent_window_decay` sample dict at collection time (not computed at commit time). Old
...
Upgrading the existing `vent_window_decay` observation at commit time keeps the pipeline
...
**Reactive-tick re-evaluation, removing the ~30-minute cadence ceiling (Issue #858).** A confirmed live recurrence on the same recency-gated path #843 fixed: a house's indoor temperature fell 6°F below the comfort floor over 2h40m before heat engaged, because `_resolve_comfort_family_via_fsm()` was only ever called from `apply_classification()`'s ~30-minute scheduled cycle. A deterministic reproduction driving `comfort_family_decision.py`/`comfort_family_fsm.py` directly with the incident's exact recorded values found no defect in that decision logic — given a genuinely-cleared recency window, it already resolves within one cycle (sustain-confirm is 90s, trivially satisfied within any 30-minute gap). The fix is therefore to remove the up-to-30-minute ceiling on when a cycle happens, not to change the decision math: `AutomationEngine.comfort_family_temperature_check()` (new) is wired into `coordinator.py`'s `_async_thermostat_changed()` — the same per-tick `current_temperature`-change event stream `nat_vent_temperature_check()`/`fan_thermostat_check()` already react to — so a floor/ceiling breach gets a chance to sustain-confirm within seconds, not up to 30 minutes later. Gated on a plausible breach against `AutomationEngine._last_comfort_band` (cached by `_apply_comfort_band()` on every call, including paused cycles) so the check is cheap on every other tick. Deliberately re-enters `apply_classification()` in full (with the cached `self._current_classification`) rather than reconstructing a shortcut path — this is what inherits, automatically and without duplication, the door/window pause choke-point inside `_apply_comfort_band()` (Issue #629) and the occupancy-aware away/vacation setback redirect (Issue #85, `decide_scheduled_band_gate()`), both re-checked fresh on every call. Test coverage: `tools/simulations/pending/issue_858_comfort_family_reactive_floor_breach.json` and its ceiling-side companion — both use only ONE `classification` event and prove the switch still lands within one sustain-confirm window, driven entirely by `temp_update` events (a revert-test of the new wiring makes both fail, confirming genuine dependence). A new DEBUG-level per-tick trace log in `_resolve_comfort_family_via_fsm()` (outcome, breach deltas, deadband applied, candidate-since ages, ODE/nat-vent/WHF/override flags) makes the next incident of this shape diagnosable directly from a log pull, closing the observability gap this investigation itself had to work around.

**Regression: the #858 guard silently disabled itself on warm/mild days (Issue #867).** The reactive check shipped in #858 checked `classification.hvac_mode not in ("heat", "cool")` and returned early whenever it was `"off"` — but `classifier.py` derives `hvac_mode="off"` for **both** `DAY_TYPE_WARM` and `DAY_TYPE_MILD` (only `hot`/`cool`/`cold` derive `"cool"`/`"heat"`/`"heat"`), while `_apply_comfort_band()` unconditionally arms a real floor/ceiling band for those same two day types (see the Warm-Day Classification Behavior note in CLAUDE.md). The guard's field check was simply wrong for exactly the population the fallback exists to protect — a real zone classified `warm` all night confirmed this via live logs: `_async_thermostat_changed()` fired dozens of times, but `_resolve_comfort_family_via_fsm()`'s trace log (added in #858) only ever appeared at the old 30-minute boundaries, proving the reactive call was returning early every time. Fixed by removing the `hvac_mode` clause entirely — applicability is decided solely by `self._last_comfort_band is None` (already present a few lines below, and already the correct, complete test `_apply_comfort_band()` itself relies on). **Why the #858 test didn't catch it:** its scenario hand-set `"hvac_mode": "cool"` on a `day_type: "warm"` classification event; `run_production.py` lets a scenario override this field directly, bypassing `classifier.py`'s real derivation, so the scenario validated a classification state production can never produce. CLAUDE.md's Golden Simulation Test Policy checklist now requires a `classification` event's hand-set fields to match real `classifier.py` derivation. Test coverage: the corrected `issue_858_comfort_family_reactive_floor_breach.json` (now `hvac_mode="off"`, `DAY_TYPE_WARM`) and the new `issue_867_comfort_family_reactive_mild_day_floor_breach.json` (same population, `DAY_TYPE_MILD`). A `DAY_TYPE_WARM`/`DAY_TYPE_MILD` *ceiling*-breach companion was considered and dropped — both day types are cooling-native (`base_family` already `"cooling"`, since `_apply_comfort_band()`'s active edge falls back to `"ceiling"` whenever `hvac_mode != "heat"`), so a ceiling breach on either is never a family *switch* (there's nothing to escalate to); the floor breach into heating is the only genuine switch case for the `hvac_mode="off"` population, and it's covered by both scenarios above. The `_natural_vent_active`/`_whf_owns_hvac()` exemption a few lines below the removed clause is unchanged by this fix and structurally independent of it (confirmed by source read, not a new test — the two are separate early-return `if` blocks in the same method).
...
| Yes, but a fan command was issued in the last 30s | — (not evaluated) | **defer** (Issue #952) | No flags touched, no `_deactivate_fan()` call, no event emitted — the mirror of the #733 row above, for the opposite read direction; see below |
| Yes (and no fan command in the last 30s) | Yes (`outdoor < indoor`, gate passes, sensors open) | **adopt-on** | `_fan_active = True`, `_natural_vent_active = True`; fast thermostatic loop started |
| Yes (and no fan command in the last 30s) | No | **turn-off** | `_deactivate_fan()` or `set_fan_mode("auto")` (FAN_MODE_HVAC) / fan `turn_off` + HVAC restore (FAN_MODE_WHOLE_HOUSE) |
...
**Issue #952 — the same stale-read race, mirrored onto the `thermostat_fan_running=True` side, made the WHF dashboard flap for 8+ hours on a live instance.** Occupant-facing symptom first: during ordinary nat-vent duty-cycling the Fan (WHF) card and the Activity Report flapped between `active` and `active (unconfirmed)` continuously, so the person in the home could not tell from the dashboard whether the fan was actually running — the physical fan itself was cycling correctly the whole time; only CA's bookkeeping was wrong. Mechanism: `nat_vent_temperature_check()` issues its normal hysteresis `_deactivate_fan(reason="nat_vent_cycling_off")`; the fan entity's state change fires `_async_fan_entity_changed()`, which schedules an immediate coordinator refresh; that refresh runs the `backstop_30min` reconcile (no cadence gate), and for a **dual-entity WHF** (separate command/feedback entities, Issue #359) the feedback entity has not caught up yet, so `thermostat_fan_running` reads a stale `True`. The `adopt-on` branch then unconditionally re-armed `_fan_active = True` seconds after CA's own deactivation, and the cycle repeated on every tick.

The fix adds one guard at a single choke point placed *after* the `no-fan` early return and *before* the nat-vent eligibility computation, so it gates **both** `thermostat_fan_running=True` branches (`adopt-on` and `turn-off`) — the same `self._is_recent_fan_command_callback(threshold_seconds=30.0)` check, the same 30s threshold, and the same "defer to the fresher command, the next trigger re-evaluates with a settled read" rationale as #733's guard one branch above. Deferral is logged at INFO (a normal mid-cycle race outcome, not a fault), matching #733's line. **Deliberately out of scope:** the RF-remote-timer sub-branches above (`remote_timer_provenance is not None and thermostat_fan_running`) read the same parameter but are one-shot with no retry path — deferring them would silently *lose* the restart-time re-arm decision rather than postpone it, so they keep their unguarded behavior. Also deferred, **not dropped**: the ~9 other independent `_is_recent_fan_command` / `_is_recent_fan_command_callback` call sites across `coordinator.py` and `automation.py` have not been audited for the same missing-sibling-guard shape; that audit needs its own tracking issue and session. See `tools/simulations/pending/issue_952_adopt_on_stale_read_deferred.json` for the regression scenario (dual-entity WHF archetype, `use_coordinator: true` — the recency callback is only wired on the real coordinator, so a bare-engine scenario would pass identically with the fix reverted).
...
#### H. Sealed-House Guard — Override Sessions Are Not Exempt (Issue #882)

Occupant impact: on 2026-09-09, all monitored doors/windows closed while a whole-house fan (WHF) RF-remote-timer override was active. The fan kept running for ~5m53s afterward — pulling outdoor air through a sealed house — until an unrelated timer expiry happened to turn it off. It self-resolved by luck, not by design.

**Root cause:** Fix D (Issue #277, `handle_all_doors_windows_closed()`) only checked `_fan_active`, which a manual/RF-remote override never sets (it sets `_fan_override_active` instead — a deliberate split so CA doesn't fight a fan the user turned on by hand). Even if Fix D's condition had fired, `_deactivate_fan()`'s Issue #486 absolute-override guard would have suppressed the shutoff unless the caller passed `bypass_absolute_override=True`. Neither gap was a regression — `handle_fan_manual_override()` predates Fix D, and Fix D's own commit scoped itself to `_fan_active` only.

**Fix:** a single choke-point method, `_enforce_sealed_house_whf_guard()`, next to `_any_monitored_sensor_open()` — stops the WHF (CA-owned **or** override) the instant no monitored sensor is open, using `bypass_absolute_override=True` (the same escape hatch Issue #748 already established for the hard AC/WHF mutex — a sealed house is the same class of hard physical invariant, not the "routine automation second-guessing" Issue #486 protects against). Because `_deactivate_fan()`'s own idempotency guard reads only `_fan_active`, the guard transiently sets `_fan_active = True` before calling it for an override-only session — the same "let `_deactivate_fan` see an owned fan" idiom `_reconcile_fan_on_startup_locked()`'s turn-off branch already uses (§B above).

Called from three sites so no WHF-activation path can bypass it:
1. **`handle_all_doors_windows_closed()`** — the event-driven sensor-closed handler (~1-3s latency, the primary path).
...
| `THERMAL_DECAY_MAX_WINDOW_MINUTES` | `60` | minutes | Wall-clock limit before `vent_window_decay` / `vent_fan_decay` abandon (H4) |
...
| `THERMAL_PASSIVE_SAMPLE_INTERVAL_S` | `300` | seconds (5 min) | Sample gate for `passive_decay`, `vent_window_decay`, and `vent_fan_decay` (H1) |
| `THERMAL_VENT_MIN_SAMPLES` | `20` | observations | Min committed samples for `vent_window_decay` / `vent_fan_decay` to populate `k_vent_window` / `k_vent_fan` (Issue #587) |
| `THERMAL_VENT_MIN_SIGNAL_F` | `0.3` | °F | Min \|T_indoor − T_outdoor\| for `vent_window_decay` / `vent_fan_decay` commit (Issue #587) |
...
| `THERMAL_SOLAR_FACTOR_MIN_RANGE` | `0.30` | — | Minimum solar_factor range (max−min) across vent_window_decay samples to trigger 2-param OLS (Issue #126) |
...
**`_end_nat_vent_session()` — sole writer of the session flags, gated on the fan command's actual result (Issue #931):** `_exit_nat_vent()` itself, plus 8 other sites that hand-write the same "clear `_natural_vent_active`/`_nat_vent_soft_start` in connection with a fan-stop command" pattern (the Priority-2/3 exit bypass branches inside `check_natural_vent_conditions()` and `nat_vent_temperature_check()`, bedtime's two no-op branches, the ceiling escalation guard, and the manual-override-conflict standdown path below, `_stand_down_whf_for_override_conflict()`), used to clear these flags **unconditionally** — before, or independent of, whether the paired `_deactivate_fan()` call actually turned the fan off. If that command came back `RATE_LIMITED_NEW`/`RATE_LIMITED_DUP` (the 300s anti-cycling floor from Issue #641), the fan stayed physically running but the flags said the session had ended — and since `nat_vent_temperature_check()` (the one function that retries a stalled pause) opens with `if not self._natural_vent_active: return`, nothing ever tried again. A live incident let a WHF run 2.5 hours past its overnight floor this way, confirmed via production logs and the real power-detect entity's history.

All 9 sites now funnel through one method:
...
def _end_nat_vent_session(self, result: FanCommandResult) -> None:
    if result in (FanCommandResult.RATE_LIMITED_NEW, FanCommandResult.RATE_LIMITED_DUP):
        return  # command never reached the fan — leave the session active for retry
...
#### Mirrored onto the economizer (Issue #936)

The economizer's `_check_window_cooling_opportunity_fsm()`/`_deactivate_economizer()` pair had the identical two gaps on its own activate/deactivate flow — a pre-await FSM-state write on activation, an unconditional flag-clear on deactivation. Fixed by `_apply_economizer_fsm_state_after_activation()` (direct mirror of `_apply_nat_vent_fsm_state_after_activation()` above) and a gated `_deactivate_economizer()` (direct mirror of `_end_nat_vent_session()` above), same fallback-state reasoning per side. See `docs/economizer-lifecycle-spec.md`'s [Rate-Limit / Override Symmetry section](economizer-lifecycle-spec.md#rate-limit--override-symmetry-issue-936-mirrors-931935) for the full writeup — not duplicated here to avoid the two specs drifting out of sync.
...
**Removed in Issue #959 — this section is now historical only.** The "proactive floor exit" mechanism this section describes (`PROACTIVE_FLOOR` in `nat_vent_exit.py`, formerly `check_natural_vent_conditions()`'s Phase 2 predictive check) has been deleted from production entirely, not merely patched again. Its predictive full-session exit, combined with a low reactivation bar, caused overnight nat-vent sessions to settle at the bare comfort floor instead of holding near their intended cycling target — a different, deeper problem than the sleep-aware-floor bug fixed above. `decide_nat_vent_exit()`'s exit chain is now 4 real checks (`COMFORT_FLOOR`, `AWAY_CEILING`, `OUTDOOR_RISE`, `CEILING_THRESHOLD`), not 5. The Issue #417/#427 fixes above remain accurate history for the mechanism as it existed at the time; do not use them as evidence that a proactive/predictive floor exit still runs in current code.
...
learning_health: {
    ...
    for obs_type in [
        ...
        "vent_window_decay", "vent_fan_decay", "solar_gain"
    ]
}
```
...
vent_window_decay   2         0          2           too_few_samples (n=2/5)
vent_fan_decay      0         0          0           —
...
vent_window_decay    monitoring  164.3 min 6        2.1 min   71.8-72.1°F      69.0°F    0.3°F

(Rejection log entries: vent_window_decay=5)
```
...
```
Thermal rolling window: obs_type=vent_window_decay n=6 elapsed=5.0min indoor=[72.0..72.0] (ΔT=0.00°F) outdoor=69.0
Thermal event commit failed (vent_window_decay): k_passive rejected (R²=0.000, n=6, indoor_ΔT=0.00°F) code=ols_bad_fit
Thermal obs abandoned [type=vent_window_decay reason=ols_bad_fit n=6/4 dt=0.00°F/? elapsed=35m]
```
...
| `pending[].obs_type` | `str` | Observation type key (e.g., `"passive_decay"`, `"vent_window_decay"`) |
