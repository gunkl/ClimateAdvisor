...
| Where can the user see how much longer an active grace period will run? | The **Debug tab** table has always shown it via inline JS formatting of `grace_end_time` and `last_action_reason` (both already in the API payload). As of Issue #620, the **Status tab**'s `_compute_automation_status()` (coordinator.py) also appends the same information — a formatted end-time + remaining-minutes suffix via `_format_grace_remaining()` (reading `_grace_end_time`, added by Issue #498) plus a reason via `_last_action_reason` (`_record_action()`), e.g. "grace period (manual) — Fan cancel (user turned off) — ends 7:14 AM (18 min left)". Issue #498 originally wired `_format_grace_remaining()` into `_compute_next_automation_action()`/`_compute_next_action()`; Issue #527 correctly stripped all mechanism-narration from those two cards per the Status Card Ontology, but the countdown was never migrated to the Status card (its correct home) until #620. | [§ Timer Lifecycle](#timer-lifecycle) |
