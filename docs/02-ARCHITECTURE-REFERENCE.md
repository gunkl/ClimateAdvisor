...
| `classification_fsm.py` (Issue #742, legacy removed Issue #757) | **None** — deliberately stateless (see the spec's own five-whys); `transition()` takes no `current_state` parameter, only composes `desired_state.decide_scheduled_band_gate()` (existing, unchanged) with `ode_ceiling_guard.decide_ode_ceiling_guard()` (Issue #742's extraction of `apply_classification()`'s former ~190-line ODE ceiling guard block) | *(none — see below)* | Sole implementation; the legacy inline eligibility/dormancy/breach-scan/lead-time block in `apply_classification()` and the `_classification_fsm_authoritative` cutover flag were removed in Phase 7 graduation (Issue #757) — the final subsystem to graduate — once the FSM path had been permanently authoritative in production for weeks with zero corpus divergence. `apply_classification()` now unconditionally dispatches through `_resolve_classification_fsm_state()`, applying the decision for real via `_apply_ode_ceiling_guard_decision()`. As of this step, neither `_engine_a` nor `_engine_b` carries any `_*_fsm_authoritative` flag — all 5 have graduated. |
