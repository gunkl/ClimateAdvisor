...
| What are the natural ventilation exit conditions and in what order are they checked? | Priority order: **1** all sensors closed → **2** comfort floor exit (sleep-aware: `sleep_heat - hysteresis` during the sleep window, `comfort_heat` otherwise, Issue #402) → **3** away-mode ceiling exit → **4** outdoor-rise exit (outdoor ≥ indoor) → **5** ceiling threshold exit (outdoor > comfort_cool + delta). First match wins. The predictive "proactive floor exit" (thermal-model prediction) that previously sat at priority 4 was removed in Issue #959 — it was ending nat-vent sessions early on a forecast, causing overnight sessions to settle at the bare comfort floor instead of their intended cycling target; the reactive comfort-floor exit and cycling-ladder thresholds already cover the safety case. During the sleep window, session-continuation cycling is handled separately by `nat_vent_temperature_check()` (fan off at `sleep_heat`, on at `sleep_heat + 2×hysteresis`) — the old "Priority 0 sleep-ceiling reached" exit was removed in Issue #371 and is not part of the current design. | [§12b. Continuous Monitoring](07-AUTOMATION-FLOWCHART.md#12b-continuous-monitoring-check_natural_vent_conditions) |
...
| Can the `no-fan` reconcile branch strand `_pre_fan_hvac_mode` even though it's specifically the fix for that stranding (Issue #405)? | Yes, under one condition fixed by Issue #618: when `_paused_by_door=True` (window still open) at the moment it fires, the old code's `restore_hvac=False` also skipped releasing the snapshot, silently re-stranding it. Fixed by decoupling release from restore — `release_suppression=True` now always fires at this genuine session-end point regardless of whether the mode write is deferred (for a door pause, or because the trigger was a normal `cooling`/`heating`→`fan` blower-phase transition). | [§14 key invariant (D)](07-AUTOMATION-FLOWCHART.md#14-fan-startup-reconciliation-and-thermostatic-loop-issue-327) |
...
| Can the economizer or the nat-vent cycler claim a fan command happened when it was actually rate-limited/deferred? | No, as of Issue #936 — both gate on the real `FanCommandResult`: the economizer's `_apply_economizer_fsm_state_after_activation()`/`_deactivate_economizer()` mirror nat-vent's own `_apply_nat_vent_fsm_state_after_activation()` (#935)/`_end_nat_vent_session()` (#931); the nat-vent cycler and away-ceiling-exit events suppress `RATE_LIMITED_DUP` re-emission and annotate `RATE_LIMITED_NEW` with a `fan_mode_change` deferral marker instead of claiming the toggle happened. | [§10 Economizer — Issue #936 gating](07-AUTOMATION-FLOWCHART.md#10-economizer--window-cooling-on-hot-days) · [§12b cycler emission gating](07-AUTOMATION-FLOWCHART.md#12b-continuous-monitoring-check_natural_vent_conditions) |
...
**FSM-equivalent (Issue #637):** the same NORMAL/PAUSED/GRACE flow above is separately modeled as a named enum, `door_window_fsm.py`'s `DoorWindowLifecycleState` (5 states — `NORMAL`, `PAUSED_ACTIVE`, `PAUSED_IDLE`, `GRACE`, `PAUSED_DURING_GRACE`). `AutomationEngine._doorwindow_fsm_authoritative` is fixed at construction as of Issue #729 (`True` on the FSM engine identity, `False` on the legacy one) — no longer an independent runtime switch; see `switch.climate_advisor_shadow_engine_primary`. See [§FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer).
...
**FSM-equivalent:** `_check_window_cooling_opportunity_fsm()` is the sole implementation of `check_window_cooling_opportunity()` as of Phase 6 — the diagram above shows the decision shape, still accurate; see [economizer-lifecycle-spec.md](economizer-lifecycle-spec.md) for the FSM transition table.

**Gated on the fan command's real result, not the pre-await decision (Issue #936):** nodes `J`/`N`/`O` (Set HVAC off / Activate fan) and `_deactivate_economizer` (nodes `D`/`H`) no longer write `_economizer_active`/`_economizer_phase` before or independent of the fan command's outcome:
- **Activation (`J`/`N`/`O` → `_economizer_active = True`):** `_check_window_cooling_opportunity_fsm()` now `await`s `_activate_fan()` first and only applies the decided phase (`MAINTAIN`/`COOL_DOWN`) via `_apply_economizer_fsm_state_after_activation()` if the fan command wasn't rate-limited or overridden mid-await; otherwise it applies `INACTIVE` so the next tick's eligibility check retries activation. Direct mirror of nat-vent's own §12a/§12c activation gating (Issue #935).
- **Deactivation (`_deactivate_economizer`):** only clears the flags once `_deactivate_fan()`'s result confirms the stop command actually ran; a rate-limited result leaves the session flags active for retry on the next tick, while the HVAC-resume step still runs unconditionally. Direct mirror of nat-vent's `_end_nat_vent_session()` gating (Issue #931).

Full writeup: [economizer-lifecycle-spec.md § Rate-Limit / Override Symmetry](economizer-lifecycle-spec.md#rate-limit--override-symmetry-issue-936-mirrors-931935).
...
**See also:** [nat-vent-lifecycle-spec.md](nat-vent-lifecycle-spec.md) (Issue #606) — the session-state view of this same lifecycle (4 named states derived from flags, differentially validated against every golden/pending scenario), plus a note on which exits above route through `_exit_nat_vent()` versus mutating flags directly. As of Issue #633, that named view has an FSM counterpart, not just a derivation: `nat_vent_fsm.py`'s `NatVentLifecycleState` enum (`INACTIVE`/`ACTIVE_FULL_GATE`/`ACTIVE_SOFT_START`/`PAUSED_REACTIVATION_LOCKOUT`, same 4 states) has a `transition()` table calling the same `decide_nat_vent_gate()`/`decide_nat_vent_exit()` pure functions this flow already uses. `AutomationEngine._natvent_fsm_authoritative` is fixed at construction as of Issue #729 (`True` on the FSM engine identity, `False` on the legacy one) — furthest-along of the three lifecycle FSMs per the Strangler Fig Atlas. See [§FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer).
...
**Cycler emission gated on the fan command's real result (Issue #936):** `nat_vent_temperature_check()`'s on/off cycling (node `J` above) and the away-ceiling exit (`B2`/`C2`) both call `_activate_fan()`/`_deactivate_fan()` and capture the returned `FanCommandResult` before emitting their Activity Report event (`nat_vent_fan_on`/`nat_vent_fan_off`/`nat_vent_away_ceiling_exit`). A `RATE_LIMITED_DUP` result (a repeat hit within an already-reported 5-min deferral window, Issue #641's anti-cycling floor) suppresses the event entirely; a fresh `RATE_LIMITED_NEW` still emits but carries a `fan_mode_change` field describing the deferral instead of implying the toggle happened immediately. See [activity-report-table.md](activity-report-table.md#nat-vent--fan) for the exact payload shape and the `_deferred_fan_mode_change_marker()` helper both sites now share.
...
- **(D) `no-fan` never restores HVAC while paused for an open door/window (Issue #523), or when the trigger was a normal post-compressor blower phase (Issue #618).** The `no-fan` branch releases any stranded WHF HVAC suppression via `_deactivate_fan(restore_hvac=not self._paused_by_door and not recent_hvac_session_ended, release_suppression=True)` — the write is skipped both if the nat-vent/`apply_classification` step immediately above just set `_paused_by_door=True` (a window is open and HVAC was already off — see §12 and `grace-periods-spec.md`) and if this reconcile was triggered by a thermostat `hvac_action` transition directly out of `cooling`/`heating` into `fan` (the thermostat's own internal blower phase, not a real unowned fan appearance — never interrupt an active/just-finished HVAC session because of it). Crucially, `release_suppression=True` always fires regardless of either guard: before Issue #618, a paused-by-door or recent-session-end skip of the write also skipped clearing `_pre_fan_hvac_mode`, silently re-stranding the same snapshot Issue #405/#418 already fixed — `thermostat_fan_running` is `False` here, so ownership must always be released even when the mode write is deferred.
