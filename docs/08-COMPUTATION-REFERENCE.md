...
| Why did overnight nat-vent flap between `nat-vent` and `paused — door/window open` every ~5 min with the window never actually closing? | All 5 nat-vent reactivation-gate call sites (`_nat_vent_may_reactivate()`'s 4 callers plus `reconcile_fan_on_startup()`'s separate hand-rolled copy) used the flat daytime `comfort_heat` floor even during the sleep window, unlike the already-sleep-aware cycling functions. Indoor sitting between `sleep_heat` and `comfort_heat` overnight repeatedly failed the wrong floor check. Fixed via a shared `_nat_vent_reactivation_floor()` helper (Issue #417). | [§17 Issue #417 — sleep-aware comfort_heat, and a 5th site folded in](08-COMPUTATION-REFERENCE.md#issue-417--sleep-aware-comfort_heat-and-a-5th-site-folded-in) |
...
| Why did the whole-house fan snap on and back off within the same minute when a monitored sensor bounced open/closed rapidly, even with a debounce configured? | `check_natural_vent_conditions()`'s idle_open reactivation branch (Issue #244/#402) reacted to the sensor's raw instantaneous state with no settle time — `CONF_SENSOR_DEBOUNCE` only ever gated the pause decision (`handle_door_window_open()`), never this path. A real incident showed a contact sensor group bouncing 7 times in 28 seconds, each open re-arming the fan before the next close tore it down. Fixed by gating idle_open on the same per-entity debounce timers (`_door_open_timers`, via new `_sensor_debounce_pending_callback`) — no second debounce concept added. Default bumped 5→10 min. | [§10 Door/Window HVAC Pause — Issue #504](08-COMPUTATION-REFERENCE.md#10-doorwindow-hvac-pause) |
| Why did the house go ~4.5 hours with no active cooling on a 95°F day even after windows closed, and why did the AC that finally started get shut off 5 minutes later? | Two symptoms, one root cause: `_deactivate_fan(restore_hvac=False)` never cleared `_pre_fan_hvac_mode` when a window was still open at genuine session-end, stranding `_whf_owns_hvac()==True` for as long as the pause lasted — `apply_classification()`'s `DEFER_NAT_VENT` gate kept deferring the HVAC-mode restore for hours after the window closed. Separately, `reconcile_fan_on_startup()`'s "no-fan" branch couldn't distinguish a normal post-compressor `cooling`→`fan` blower-phase transition from a real unowned fan appearance, so it blindly replayed that same stale snapshot and force-cancelled AC that had just started legitimately cooling. Fixed by `_deactivate_fan()`'s new `release_suppression` parameter (always clears the snapshot at a genuine session end, independent of whether a mode gets written) and `reconcile_fan_on_startup()`'s new `recent_hvac_session_ended` parameter (never writes a mode when the trigger was a normal blower-phase transition). | [§9a Fan State Tracking](08-COMPUTATION-REFERENCE.md#9a-fan-state-tracking) · [§9e Thermostatic Fan Loop and Startup Reconciliation](08-COMPUTATION-REFERENCE.md#9e-thermostatic-fan-loop-and-startup-reconciliation-issue-327) |
| Why did the AC get set to cool with a monitored window still open, 5 seconds after the user turned the WHF off manually? | Three independent gaps, not one: (1) `check_natural_vent_conditions()`'s `_idle_open` widening (Issue #244/#402/#504) never checked `_grace_active`, letting a fan-off grace be bypassed and the fan reactivated within seconds — fixed by excluding `_grace_active` from `_idle_open`. (2) `fan_thermostat_check()`'s `STOP_DEACTIVATE`/`STOP_COOLED_TO_FLOOR` outcomes (unlike their sibling `STOP_VIA_NAT_VENT_EXIT`, migrated by Issue #418) still called bare `_deactivate_fan()` with no live sensor check — fixed by routing both through `_exit_nat_vent()` too. (3) The deepest gap: `decide_scheduled_band_gate()`'s `paused_by_door` input reflected only event history, never live sensor state — a sensor open since before any pause-triggering event left it `False` forever, so routine `apply_classification()`/`handle_bedtime()`/`handle_morning_wakeup()`/`handle_pre_cool()` writes had no live check at all. Fixed by a new `_sync_paused_by_door_with_live_sensors()` helper, called at the top of all four, reusing the existing `_pause_for_door_window()` and the same debounce-settle property `_idle_open` already used. | [§9. Structural WHF/AC Mutual Exclusion](08-COMPUTATION-REFERENCE.md#9-structural-whfac-mutual-exclusion) · [Shared Scheduled-Band Gate (Issue #498)](grace-periods-spec.md#shared-scheduled-band-gate-issue-498) |
| Why did briefly opening a monitored door (e.g. walking outside) trigger an instant "HVAC paused" notification, bypassing the configured 10-minute debounce window entirely? | Regression in Issue #620's Fix A (`_sync_paused_by_door_with_live_sensors()`, PR #621). Its debounce guard, `_sensor_debounce_pending`, delegated to `lambda: bool(self._door_open_timers)` — true only once the `state_changed` event listener (`_async_door_window_changed()`) has registered a timer for that transition. But raw sensor reads (`_any_monitored_sensor_open()`) reflect the open instantly, independent of listener scheduling — an unrelated, concurrently-running coordinator refresh cycle could read "open, no timer yet" before the listener's turn and misfire a pause. Confirmed live 2026-08-11: the pause logged 5ms before "debounce started" for the same sensor. Fixed by making `_sensor_debounce_pending()` also check each open sensor's HA-authoritative `last_changed` timestamp, immune to scheduling order. | [§ Debounce-Timer Registration Race (Issue #623)](grace-periods-spec.md#debounce-timer-registration-race-issue-623) |
| Why did the AC get silently commanded into "cool" mode with a monitored window still open, over an hour after nat-vent released ownership, with no pause and no log line even mentioning the mode change? | Not an external override — CA itself did it. `_set_temperature()` (Issue #301 single-setpoint architecture) bundles `hvac_mode` into every `climate.set_temperature` call. `select_comfort_band()`'s two-way ternary (`active = "floor" if hvac_mode == "heat" else "ceiling"`) treats an "off"-day classification the same as a cool day, by design, for the "lazy comfort band" safety net (Issue #249) that arms a wide band on any day type once windows are closed. The only thing that should stop this write is `_paused_by_door` — upstream flag bookkeeping that can lag: a coordinator refresh fired `apply_classification()` moments after nat-vent released ownership, before any door-pause reconciliation had a reason to run, and `_apply_comfort_band()` had no independent check of its own. Separately, `_set_temperature()`'s log line never mentioned the `mode` it silently sent, which is why the mode change was invisible in logs. Fixed by (1) a structural choke-point guard directly in `_apply_comfort_band()` — the single write point all 7 comfort-band callers funnel through — refusing to arm an active mode while a monitored window is genuinely (debounce-settled) open, mirroring the WHF/AC mutex choke-point already inside `_set_hvac_mode()` (Issue #392 Fix 1b); exempted while nat-vent/WHF genuinely owns HVAC, since `handle_occupancy_away()`/`handle_occupancy_vacation()` legitimately arm a wide, inert setback band during an active nat-vent session. (2) `_set_temperature()`'s log line now includes `mode=`. | [§9. Structural WHF/AC Mutual Exclusion](08-COMPUTATION-REFERENCE.md#9-structural-whfac-mutual-exclusion) |
| Why did HVAC mode flip to "cool" right after a redeploy/restart, with the dashboard still correctly showing "windows open" and the door/window sensor genuinely open the whole time? | A monitored group/helper contact-sensor entity blips `unavailable → on` during HA startup (confirmed via live REST state history: transitioned right across a redeploy), stamping a fresh `last_changed` on a window that had physically been open for hours — `_sensor_debounce_pending()`'s `last_changed` fallback (Issue #623) then read "pending" for up to `CONF_SENSOR_DEBOUNCE` (default 10 min), which let the Issue #629 choke-point guard in `_apply_comfort_band()` skip the pause and arm the band. A first fix attempt (blanket-removing the debounce check from the guard) broke the golden `issue_623_debounce_race_transient_open_not_paused` scenario — debounce's legitimate protection against a nuisance pause on a genuinely brief door-open. Fixed instead by teaching `coordinator._async_door_window_changed()` to recognize an `unavailable`/`unknown` → open transition as a reconnect blip (not a real open) and exclude its `last_changed` from `_sensor_debounce_pending()`, leaving the arm-blocking guards themselves untouched. `_idle_open`'s debounce check (Issue #244/#402/#504, reactivation-widening) is unaffected — "wait and see" remains correct there regardless of blip-vs-genuine. | [§9. Structural WHF/AC Mutual Exclusion](08-COMPUTATION-REFERENCE.md#9-structural-whfac-mutual-exclusion) |
...
**Restart persistence for FSM-authoritative switches, and shadow-engine promotion (Issue #727).** Two related changes to the runtime control model above the mechanisms described in this section:

1. `switch.climate_advisor_nat_vent_fsm_authoritative` / `..._door_window_fsm_authoritative` / `..._override_grace_fsm_authoritative` were originally designed to reset to legacy/off on every Home Assistant restart, as a Phase R rollout safety guarantee (a restart always comes back up on the proven legacy path). Issue #727 reverses this at the owner's explicit request: all 3 now persist their last-set value across a restart — a top-level `fsm_authoritative` dict in the state file, restored in `async_restore_state()` **before** the same-day gate (unlike most of that function's restore logic, which discards state from a prior calendar day) — since these are mode-like settings, not daily ephemeral state.

2. `coordinator.automation_engine`/`shadow_automation_engine` were plain instance attributes fixed at construction — the shadow engine (§9 below) could never issue real commands, only compare forever. They are now routing properties over two fixed engine handles (`_engine_a`/`_engine_b`), selected by `self._shadow_is_primary`, so every existing read call site (`api.py`, `sensor.py`, `coordinator.py` itself) keeps working unchanged. A new `switch.climate_advisor_shadow_engine_primary` (also persisted, same reasoning as #1) calls `async_set_shadow_engine_primary()`, which does one more full input sync plus an explicit copy of the command-tracking/echo-suppression fields and the 3 FSM-authoritative flags — none of which `_sync_shadow_inputs()` has ever covered, since they're populated only inside the real (non-dry-run) branches only production could reach — before swapping `dry_run`/callback bundles/`role` on both engines. **Known, accepted limitation** (logged at WARNING every time the switch is used, not blocked on): the shadow engine's decision coverage is a strict subset of production's (see the un-mirrored entry-point list earlier in `coordinator.py`), so a decision only the demoted engine's un-mirrored code path made may not fire identically on the newly-primary engine until that coverage gap is separately closed.
...
|---|---|---|---|
| No | — | **no-fan** | `_fan_active`/`_fan_on_since`/`_natural_vent_active` cleared, then `_deactivate_fan(restore_hvac=not self._paused_by_door and not recent_hvac_session_ended, release_suppression=True)` is called to release any stranded WHF HVAC suppression (Issue #405, extended by Issue #618) |
