...
| 2 | `check_natural_vent_conditions()` | Grace re-entry branch — reactivation after a grace period; also covers the Issue #134 comfort-ceiling re-entry check, folded into this same choke point in Issue #411 as a 4th duplicate found during the reactivation-gate extraction |
| 3 | `nat_vent_temperature_check()` | Paused-state reactivation |
