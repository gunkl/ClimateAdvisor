...
**FSM-equivalent (Issue #637, shadow-only):** the same NORMAL/PAUSED/GRACE flow above is separately modeled as a named enum, `door_window_fsm.py`'s `DoorWindowLifecycleState` (5 states — `NORMAL`, `PAUSED_ACTIVE`, `PAUSED_IDLE`, `GRACE`, `PAUSED_DURING_GRACE`). It runs alongside production as a diagnostic comparison; `AutomationEngine._doorwindow_fsm_authoritative` (off by default, not persisted across restart) is the switch that would let it actually drive `_paused_by_door`/`_grace_active` instead of the flag logic below. See [§FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer).
...
**FSM-equivalent (Issue #639, shadow-only):** the override-confirmation half of this flow maps to `override_grace_fsm.py`'s composite `(OverrideConfirmState, GraceState)` state — `OverrideConfirmState.PENDING` covers the confirmation window (nodes E/K below), `IDLE` otherwise. It runs as a diagnostic comparison only; `AutomationEngine._override_grace_fsm_authoritative` (off by default, not persisted across restart) would be required before it could drive real state. See [§ Grace Period System](#8-grace-period-system) below and [§FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer).
...
**FSM-equivalent (Issue #639, shadow-only):** `override_grace_fsm.py`'s `GraceState` (`NONE`/`ACTIVE_PROTECTING_OVERRIDE`/`ACTIVE_UNPROTECTED`) names whether grace is running and, separately, whether it's protecting a real override — the same distinction `_grace_protects_override` (Issue #530) already draws in the flag logic below. Diagnostic-only comparison today; `AutomationEngine._override_grace_fsm_authoritative` (off by default, not persisted across restart) is the not-yet-flipped cutover switch. See [§FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer) and [grace-periods-spec.md](grace-periods-spec.md).
