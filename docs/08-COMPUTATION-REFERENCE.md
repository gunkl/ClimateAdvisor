...
**Unified exit handoff — `_exit_nat_vent()` (Issue #411):** All 4 exit paths above (Priority 1 sensor-close, Priority 2 comfort-floor hard exit, Priority 3 outdoor-rise exit, plus `apply_classification()`'s proactive/predictive floor exit that projects a coming floor breach from the thermal model's `k_passive` rate) now hand off through one function, `_exit_nat_vent(self, *, reason: str, set_outdoor_exit_time: bool = False) -> None`, instead of each call site independently deciding whether to restore HVAC or pause. `_exit_nat_vent()` clears `_natural_vent_active`, then checks the monitored sensor state itself: if a sensor is still open, it pauses (`_deactivate_fan(restore_hvac=False)`, sets `_paused_by_door=True`, captures `_pre_pause_mode`) rather than restoring HVAC into an open window; if sensors are closed, it restores HVAC (`_deactivate_fan(restore_hvac=True)`, the default) and starts a grace period. Only the outdoor-reversal (Priority 3) call site passes `set_outdoor_exit_time=True`, since only that path's lockout timer (`_nat_vent_outdoor_exit_time`, consumed by the reactivation gate below) depends on it — the other 3 folded-in paths do not gain or lose that side effect from the unification. Before this fix, the proactive/predictive exit hand-rolled its own restore (`_set_hvac_mode(c.hvac_mode)` using the day classification's mode, regardless of sensor state) on top of `_deactivate_fan()`'s already-correct restore, and never checked whether a monitored door/window sensor was still open — producing a contradictory log narrative (nat-vent "exiting" and re-entering repeatedly for hours) when a window stayed open overnight (Issue #411). Routing it through `_exit_nat_vent()` removes both the sensor-blind override and the double-restore, and also gives the outdoor-too-warm exit a `_pre_pause_mode` capture it never had before. Away-mode ceiling exit is intentionally **not** routed through `_exit_nat_vent()` — it has no pause/grace state machine and remains a separate, simpler path by design.
