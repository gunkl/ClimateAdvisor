...
### 6f. Comfort-Floor Defense on Cool-Classified Days (Issue #821)

**Root cause this closes:** §6e's band model (`select_comfort_band()`) picks `active` purely from the day's classification (`classification.hvac_mode`), never from current indoor temperature — a day classified `cool` never arms a heat command, even if indoor has decayed well below `comfort_heat`, unless nat-vent is riding it down and exits on its own priority chain (§9). A zone with no thermal-model history yet (`confidence_k_passive == "none"`) has no ODE guard available either (§6c requires a calibrated `k_passive`). Confirmed live: Zone "Simulated 2" sat at 57°F for 2+ hours while CA reconfirmed a 74°F cool setpoint every cycle.

**The shared resolver:** `_resolve_comfort_family_mode(day_mode, now)` in `automation.py` is the single decision point for "should the heating or cooling family be active right now," consumed by **both** `_apply_comfort_band()` (§6e) and `_set_temperature_for_mode()` — the latter is itself the sole choke point for its 7 callers (TOU pre-conditioning, door/window-all-closed resume, `check_natural_vent_conditions()`'s own COMFORT_FLOOR restore, `resume_from_pause()`, occupancy-home restore, economizer deactivation, and `_exit_nat_vent()`'s sensors-closed restore branch), so wiring the resolver in there once covers all 7 without per-site changes — **except** TOU pre-conditioning: a caller passing `target_override` bypasses the resolver entirely and uses `c.hvac_mode` directly (see below). It only ever escalates `cool → heat` (never the reverse — ceiling breaches on `off`-classified days remain §6c's territory); it defers entirely while nat-vent/WHF owns HVAC or a manual override is active.

**TOU pre-conditioning exception (Issue #821 Verification BLOCKING #3):** TOU's own `target_override` is deliberately a cool-mode banking value at or below `comfort_heat`, written with `skip_setpoint_sanity_check=True` — the one safety check that would normally flag a heat-mode command at a below-floor setpoint. If the resolver were consulted for this call site, it could independently escalate to `heat` at TOU's own below-floor cool target with that safety check turned off. `_set_temperature_for_mode()` therefore skips the resolver whenever `target_override is not None` and uses `c.hvac_mode` as before Issue #821 — TOU's own `decide_scheduled_band_gate()` is already the authority for whether that call site should proceed.

| `confidence_k_passive` | Path | Escalates when |
|---|---|---|
| `"none"` | **Fallback** (Design §3) | `indoor < comfort_heat`, sustained `COMFORT_FALLBACK_CONFIRM_S` (90s) |
| not `"none"` | **ODE floor guard** (`ode_floor_guard.py`, mirrors §6c) | Cached `comfort_floor_crossing_time` (from `nat_vent_plan.py`, Issue #817 single source of truth) within a `k_active_heat`-sized lead-time buffer |

**`_exit_nat_vent()`'s own restore path:** `_deactivate_fan()`'s restore branches (both "fan already inactive" and "fan was active") restore HVAC mode via a blind replay of `self._pre_fan_hvac_mode` — whatever mode was active *before* nat-vent started — never consulting the shared resolver. Since nat-vent only ever starts on a cooling day, this almost always restores `"cool"`, reproducing the original bug through nat-vent's own most common exit path: `PROACTIVE_FLOOR`/`OUTDOOR_RISE`/`CEILING_THRESHOLD` and a plain window-close all route through `_exit_nat_vent()`, not through `check_natural_vent_conditions()`'s separate `COMFORT_FLOOR` branch (which already calls `_set_temperature_for_mode()` on its own). Fixed by adding a follow-up `_set_temperature_for_mode(self._current_classification, ...)` call right after `_deactivate_fan()` in the sensors-closed restore branch only (mirroring the door/window-all-closed restore site and `resume_from_pause()`) — the sensors-still-open pause branch is untouched, since it correctly restores nothing.

**Anti-flap mechanisms (both required, distinct durations — do not conflate):**
- **Sustain-confirmation** (`confirmed_transition.py`, `NAT_VENT_EXIT_SUSTAIN_S` = 90s): a candidate must read true continuously before committing, not on a single instantaneous reading. Applied to `decide_nat_vent_exit()`'s 5 exit reasons (`nat_vent_exit.py`) at both real call sites (`check_natural_vent_conditions()`, `nat_vent_temperature_check()`) — all exempt only `MANUAL_OVERRIDE_CONFLICT`.
  - **Deliberately NOT applied to `fan_thermostat_check()`'s own Check 1/Check 2** (`fan_thermostat_decision.decide_fan_thermostat_check()`, fired on every indoor/outdoor change — the highest-frequency nat-vent-exit trigger in the codebase). This is a reasoned design boundary, not an oversight: that loop exists specifically for rapid response to prevent real overshoot (Issue #327's airflow-reversal stop, Issue #402's overcooling stop) — delaying it 90s would trade flap-reduction for reintroducing the exact overshoot bugs it was built to fix. Its stops already arm the same 300s reactivation lockout (`_exit_nat_vent()`'s `set_outdoor_exit_time=True`) every sustain-confirmed exit reason also uses, so re-entry is still debounced even though the exit itself is instantaneous. It also wasn't implicated in the reported live incident that motivated this issue — that traced to `PROACTIVE_FLOOR`, a slow-loop-only reason this method never evaluates. Confirmed by the project owner after independently reproducing a gating attempt (it broke 6 additional locked golden scenarios) and tracing this method's own history and lockout coverage directly — see `docs/issue-821-golden-scenario-review-notes.md` for the investigation.
- **Family-switch lockout** (`comfort_mode_switch_min_interval_s`, default 600s, advanced config): once escalated, a switch back requires this minimum dwell since the family *genuinely started* (`_arm_comfort_family()`). Nat-vent/WHF activation and deactivation (`_exit_nat_vent()` and its bypass branches) always re-arm the dwell clock — each fires once per real physical event, so a fresh window at that transition is correct (this is what makes "exiting nat-vent counts as cooling family was active until just now" work). `_apply_comfort_band()` and `_set_temperature_for_mode()` instead pass `only_if_changed=True`: since those run on every classification cycle regardless of whether anything changed, routine reassertion of an already-recorded family is a no-op and does NOT reset the clock (Issue #823 — the prior "reset on every call" behavior let routine reassertion push the dwell clock forward indefinitely, permanently blocking a legitimate escalation whenever the classification cadence was shorter than the lockout window). Logged as `comfort_family_switch_locked_out` when a genuine escalation is blocked.

**Test coverage:** `tests/test_confirmed_transition.py` (sustain-confirmation primitive); `tests/test_ode_floor_guard.py` (mirrors `test_ode_ceiling_guard.py`); `tests/test_nat_vent_exit.py`, `tests/test_nat_vent_activation.py`, `tests/test_nat_vent_thermostat.py`, `tests/test_natvent_away_ceiling.py`, `tests/test_nat_vent_soft_start.py`, `tests/test_fan_control.py::TestNatVentComfortFloorExit` (sustain-confirmed exit reasons); `tests/test_fan_control.py::TestExitNatVentRestoreConsultsResolver` (the `_exit_nat_vent()` restore-path fix — direct unit coverage proving `_set_temperature_for_mode()` is/isn't called from exactly the right branch); `tests/test_comfort_mode_family_lockout.py` (direct unit tests for `_family_switch_locked_out()`/`_arm_comfort_family()`, plus a genuine negative-control test reproducing real heat/cool-family hunting with the lockout disabled and confirming it's prevented when enabled — revert-test-verified); `tests/test_scenario_e_sustain_confirmation_negative_control.py` (loads `tools/simulations/pending/issue_821_scenario_e_sustain_confirmation_noise_rejection.json` and reruns it with `_confirm_nat_vent_exit_candidate()` patched to bypass the sustain clock, confirming the same noisy-flicker pattern that survives WITH sustain-confirmation instead commits an exit on the very first flicker WITHOUT it — revert-test-verified); `tests/test_tou_precondition.py::TestTouPreconditionResolverBypass` (proves `target_override` callers bypass the family resolver even when escalation would otherwise fire, closing Verification BLOCKING #3); `tools/simulations/pending/issue_821_comfort_floor_fallback_no_confidence.json` (the reported incident — confidence-none fallback, including the sustain-not-yet-confirmed intermediate state); `tools/simulations/pending/issue_821_natvent_floor_exit_restore_no_flap.json` and `tools/simulations/pending/issue_821_exit_nat_vent_restore_defends_floor.json` (the nat-vent floor-exit restore call site, end to end — the unit tests above are what actually discriminate the restore-path fix, since a later grace-period expiry in these end-to-end scenarios independently re-triggers the same resolver regardless of it). The 5 golden scenarios retimed for sustain-confirmation (`away_natvent_exits_at_comfort_ceiling`, `issue-359-fan-state-machine`, `issue_637_paused_during_grace_nat_vent_exit`, `nat_vent_thermostat_cycling`, `window_close_stops_whole_house_fan`) cover the sustain-confirmed slow/fast-loop exit paths end to end (their `expect` assertions alone were shown by Verification to pass with or without sustain-confirmation, since retiming removed the only signal that would catch its removal — `tests/test_scenario_e_sustain_confirmation_negative_control.py` is what actually discriminates the mechanism now).
