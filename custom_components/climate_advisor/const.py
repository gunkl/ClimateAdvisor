...
RELEASE_NOTES: dict[str, list[str]] = {
    ...
    "0.6.73": [
        ...
        " subsystem in the migration. The FSM-based ODE ceiling guard has been"
        " production-authoritative for weeks with zero corpus divergence, so the"
        " ~190-line legacy eligibility/dormancy/breach-scan/lead-time block and the"
        " differential-comparator scaffolding are no longer needed — no automation"
        " subsystem in Climate Advisor carries a legacy/FSM cutover flag anymore."
        " No new production bugs were found.",
    ],
    "0.6.72": [
    ...
    "0.6.69": [
        ...
        " FSM-based door/window dispatch has been production-authoritative for weeks"
        " with zero corpus divergence, so the old inline flag-write branch and its"
        " differential-comparator scaffolding are no longer needed.",
        "Fix #762: found while forcing every test through the door/window FSM path"
        " for the first time — two FSM input builders defaulted an unconfigured"
        ' fan_mode to "whole_house_fan" instead of the disabled default every other'
        " fan_mode read in the file uses, which could silently let a home with no"
        " whole-house fan configured skip a door/window pause it should have taken.",
        "Fix #762: the choke-point guard that refuses to arm an active HVAC mode"
        " through an open window (_apply_comfort_band()) could, in a narrow timing"
        " window right after the whole-house fan was turned off manually, fail to"
        " mark the pause visible on the dashboard even though HVAC was correctly"
        " being held off. Fixed.",
        "Fix #762: after grace expired while paused mid-grace, a genuine nat-vent"
        " reactivation could be missed due to a stale pause flag. Fixed — all three"
        " #762 fixes were latent design gaps in the FSM path since it was first"
        " built, invisible until this step made the path unconditional; no known"
        " live incident from any of them.",
    ],
    "0.6.68": [
    ...
    "0.6.67": [
        ...
        " been production-authoritative since Phase 5, so the 17 legacy closures and"
        " their differential-comparator scaffolding are no longer needed.",
        "Fix #759: the whole-house fan's fast, tick-level thermostatic stop check had"
        " been silently inert since Phase Fan/WHF went live — it never actually"
        " compared live indoor/outdoor temperatures, so the fan only stopped via the"
        " slower 30-minute cycle instead of immediately when conditions no longer"
        " favored it. Fixed; the fan now stops as promptly as originally intended.",
        "Fix #759: the whole-house fan's physical-drift self-correction was firing"
        " one backstop tick early (after ~5 minutes instead of the intended two"
        " consecutive ticks / ~10 minutes), reducing its protection against momentary"
        " sensor flaps. Fixed.",
        "Fix #759: restored a missing log line explaining why a too-soon fan on/off"
        " reversal was suppressed by the anti-flap rate limiter — the suppression"
        " itself was always working correctly, this only affects log visibility.",
    ],
    "0.6.66": [
...
KNOWN_FIXES: dict[int, dict] = {
    ...
    757: {
        ...
        "title": (
            ...
            " (fan/whole-house-fan, 0.6.67), Step 3 (override/grace, 0.6.68), and Step 4"
            " (door/window pause/grace, 0.6.69) each removed a legacy (pre-FSM) code"
            " path from AutomationEngine, once that subsystem's own"
            " _<name>_fsm_authoritative flag had been permanently True in production"
            " for weeks with zero divergence (offline differential comparator, full"
            " golden+pending corpus). Step 2's own scope_covered below documents its"
            " fan/WHF work in full; this entry's Step 3/Step 4 paragraphs document"
            " override/grace and door/window respectively."
        ),
        "scope_covered": (
            "automation.py: deleted the __init__ default self._fan_fsm_authoritative"
            " flag; _resolve_fan_fsm_state() dropped its legacy: Callable[[],"
            " FanTransition] parameter and now unconditionally runs fan_fsm.transition();"
            " deleted the legacy closure at each of the 17 real call sites"
            " (_activate_fan, _deactivate_fan x2 rate-limit gates, handle_fan_manual_"
            " override, on_fan_turned_off x2, clear_fan_override, _clear_fan_flags_"
            " and_start_grace, _stop_fan_min_runtime_cycles, _fan_cycle_on/_off,"
            " fan_thermostat_check, _thermo_backstop_task, _suppress_hvac_for_whf,"
            " _release_whf_and_reclassify x2); simplified the 2 directly-gated"
            " `if self._fan_fsm_authoritative and ...DEFER_NEW` rate-limit conditionals"
            " to drop the now-always-true predicate. One closure (_reconcile_fan_"
            "physical_drift()'s DRIFT_TICK CORRECT-outcome handler) was found to"
            " contain real side effects — grace-period start, WHF release+reclassify,"
            " and the hardware off-command — with no FSM-branch equivalent; restored"
            " unconditionally as a shell block (matches the WHF_SUPPRESSION_REQUESTED"
            " Group-1 'real write first, dispatch purely for mirror-sync' pattern"
            " already used elsewhere in this file). Removing the flag-gate around it"
            " forced every test through the FSM path for the first time (tests"
            " previously defaulted to the legacy=False path), which surfaced 3"
            " genuine pre-existing production bugs in the FSM path itself, silently"
            " live since Phase Fan/WHF (#731) went authoritative — see Issue #759 for"
            " full detail: (1) fan_thermostat_check()'s FSM dispatch call never"
            " received indoor/outdoor overrides, so its tick-level stop check always"
            " returned KEEP; (2) the DRIFT_TICK dispatch fed the FSM's own independent"
            " re-derivation the already-advanced tick count, confirming physical drift"
            " one tick early; (3) the rate-limit-suppression INFO/DEBUG log had no"
            " FSM-path equivalent. All three fixed in this same PR (not deferred),"
            " plus tools/sim_harness/fan_thermostat_decision_integration.py's"
            " break_fan_thermostat_decision() repointed to patch"
            " fan_fsm.decide_fan_thermostat_check (the module that now actually calls"
            " it) instead of automation.py's now-dead copy of the same name, and that"
            " now-genuinely-unused import removed from automation.py."
            " coordinator.py: removed the _fan_fsm_authoritative=False/True"
            " assignments on _engine_a/_engine_b; removed the fan_mirror axis from"
            " _SHADOW_DIAG_AXES and its full block in"
            " _update_shadow_engine_diagnostic() (agreement computation, debounce"
            " entry, dict assembly, WARNING logging); removed the 6 now-unused"
            " raw-copy fields from _sync_shadow_inputs() (_fan_on_since,"
            " _fan_min_runtime_active, _fan_toggle_command_time, _fan_rate_limited_"
            "until/_direction, _fan_drift_tick_count) — _fan_active/_pre_fan_hvac_mode"
            " were kept, since they independently feed _whf_owns_hvac()'s door/window-"
            " mirror correctness (Issue #716/#724), unrelated to the fan_mirror axis."
            " All 12 fan-specific _mirror_to_shadow() call sites were investigated"
            " individually and kept — each is load-bearing for the nat-vent FSM"
            " evaluator, the override/grace FSM evaluator, or Issue #716's own"
            " shadow-state-honesty enforcement test, independent of fan_mirror."
            " sensor.py: removed fan_production_state/fan_shadow_state/"
            "fan_mirror_agrees from the shadow-engine diagnostic sensor's"
            " extra_state_attributes. Deleted tests/test_fan_fsm_authoritative_"
            "compare.py and tools/sim_harness/fan_fsm_authoritative_compare.py. The"
            " separate tools/sim_harness/fan_thermostat_decision_compare.py chain (5"
            " files, an earlier and unrelated pure-function-extraction proof, Issue"
            " #716) was investigated and left untouched — it contains no"
            " FSM-authoritative-specific code. test_shadow_engine_coverage.py's"
            ...
            " site."
            " Step 4 (0.6.69): automation.py: deleted the __init__ default"
            " self._doorwindow_fsm_authoritative flag;"
            " _resolve_door_window_pause_flags() dropped its legacy: Callable[[], None]"
            " parameter and now unconditionally runs door_window_fsm.transition();"
            " deleted the legacy closure at each of the 14 real call sites"
            " (_pause_for_door_window, handle_door_window_open,"
            " handle_all_doors_windows_closed, the 4 PAUSED_NAT_VENT_REACTIVATED sites"
            " in check_natural_vent_conditions, handle_manual_override_during_pause,"
            " resume_from_pause, the 3 GRACE_TIMER_EXPIRED sites in"
            " _on_grace_expired, and _exit_nat_vent's sensor-still-open branch), plus"
            " a 15th real call site the original investigation undercounted (a second,"
            " nested if/else on the removed flag inside _re_pause_for_open_sensor(),"
            " whose dead 'not authoritative' branch called the full"
            " _pause_for_door_window() wrapper redundantly)."
            " Two genuine pre-existing bugs in door_window_fsm.py's SENSOR_OPENED"
            " transition were found and fixed while forcing every test through the"
            " unconditional FSM path for the first time (Issue #759's own precedent —"
            " see this version's RELEASE_NOTES for the occupant-facing summary of"
            " both): (1) the fan_mode default mismatch"
            " (_build_door_window_fsm_inputs()/_build_nat_vent_fsm_inputs() both"
            " defaulted to 'whole_house_fan' instead of FAN_MODE_DISABLED like every"
            " other fan_mode read in the file — added a"
            " DoorWindowFsmInputs.nat_vent_gate_ruled_out field is unrelated to this"
            " specific fix, just fixed the literal default value in both builders);"
            " (2) _pause_for_door_window()'s two real callers (_apply_comfort_band()'s"
            " choke-point guard, _sync_paused_by_door_with_live_sensors()) both call it"
            " only after already independently ruling out nat-vent AND (for"
            " _apply_comfort_band() specifically) deliberately overriding the"
            " planned-window exemption — but SENSOR_OPENED's transition logic"
            " independently re-derives both conditions, which could silently produce"
            " DoorOpenResponse.NAT_VENT_ELIGIBLE or PLANNED_WINDOW_NOOP instead of"
            " PAUSE, leaving _paused_by_door=False even though HVAC was correctly"
            " being held off. Fixed via the new"
            " DoorWindowFsmInputs.nat_vent_gate_ruled_out field (default False,"
            " True only for _pause_for_door_window()'s own dispatch), which forces"
            " both re-derived conditions off in _transition_from_normal's and"
            " _transition_from_grace's SENSOR_OPENED handling — caught live by golden"
            " scenario issue_629_comfort_band_arm_through_open_window."
            " A third, related gap was found in _re_pause_for_open_sensor() itself"
            " (not door_window_fsm.py): its non-natvent-FSM reactivation branch"
            " initially collapsed to reading the already-applied _paused_by_door flag"
            " directly (matching a pre-existing 'Issue #660 Step 8' comment's claim"
            " that _on_grace_expired()'s dispatcher call always pre-applies the"
            " reactivation decision to that flag) — true only from a plain-GRACE"
            " origin; _on_grace_expired()'s GRACE_TIMER_EXPIRED handling for the"
            " PAUSED_DURING_GRACE composite state always lands on a paused sub-state"
            " regardless of the nested nat-vent outcome, by design, so reading"
            " _paused_by_door there sees a stale True even when nat-vent should"
            " reactivate. Reverted to always independently recomputing via"
            " _nat_vent_may_reactivate() (correct regardless of origin), and restored"
            " the full _pause_for_door_window() wrapper call (not just the action"
            " half) for the blocked-reactivation branch — caught live by"
            " tests/test_resume_from_pause.py's"
            " test_repause_clears_paused_by_door_on_nat_vent_reactivation and 2"
            " siblings, plus test_door_window.py's"
            " TestRePauseForOpenSensorFsmAuthoritative (rewritten to test the"
            " corrected behavior)."
            " coordinator.py: removed the _doorwindow_fsm_authoritative=False/True"
            " assignments on _engine_a/_engine_b; removed the"
            " door_window_mirror/door_window_fsm axis pair from _SHADOW_DIAG_AXES and"
            " their full block in _update_shadow_engine_diagnostic(); removed"
            " _door_window_fsm_state, _evaluate_door_window_fsm(),"
            " _evaluate_door_window_fsm_nat_vent_exit(), and the"
            " _DOOR_WINDOW_FSM_EVENT_KINDS/_DOOR_WINDOW_SYNC_RECONCILE_TRIGGER_"
            "METHODS/_DOOR_WINDOW_GRACE_EXPIRY_EVENT_TYPES/_DOOR_WINDOW_NAT_VENT_EXIT_"
            "EVENT_TYPES/_DOOR_WINDOW_NAT_VENT_REACTIVATED_EVENT_TYPES constants that"
            " fed them — unlike Step 3's override/grace precedent, this underlying"
            " shadow-FSM-replay machinery WAS removed this step, since it had zero"
            " other consumers (confirmed by grep before deletion), retiring the"
            " known-inert PAUSED_NAT_VENT_REACTIVATED gap in"
            " door_window_fsm.py's _transition_from_grace() as a side effect (never a"
            " real-production bug — only the deleted shadow replica tracker was"
            " affected). A 15th real call site of the removed"
            " _evaluate_door_window_fsm() was also found and removed"
            " (_check_orphaned_grace()'s own re-notify call). _sync_shadow_inputs()'s"
            " 14 override/grace-era raw-copy fields were re-audited and kept in full,"
            " unchanged — nat-vent's own still-active mirror dependency alone"
            " (documented in Step 3's paragraph) already justified keeping them; the"
            " door/window mirror dependency Step 3 also cited is now gone, but was"
            " never the sole reason. sensor.py: removed"
            " door_window_production_state/door_window_shadow_state/door_window_fsm_"
            "state/door_window_mirror_agrees/door_window_fsm_agrees from the"
            " shadow-engine diagnostic sensor's extra_state_attributes (unlike Step"
            " 3's override/grace fields, which Step 3 left exposed-but-permanently-"
            "None since their underlying machinery wasn't removed). Deleted"
            " tests/test_doorwindow_fsm_authoritative_compare.py,"
            " tools/sim_harness/doorwindow_fsm_authoritative_compare.py,"
            " tests/test_door_window_fsm_shadow_wiring.py, and"
            " tests/test_shadow_fsm_harness_event_coverage.py (399 lines, entirely"
            " door/window-shadow-FSM-specific, not explicitly named in the original"
            " task but discovered to test only the removed machinery)."
            " test_shadow_engine_coverage.py's"
            " _DOOR_WINDOW_EVENT_KIND_REGISTRY/TestDoorWindowFsmEventCoverage removed"
            " (same reasoning as override/grace's own registry removal — tested"
            " exactly the now-removed axis, not an independent dispatch-site coverage"
            " concern)."
        ),
    },
    759: {
        ...
        "title": (
            "3 silent production bugs in the fan/WHF FSM-authoritative path, found"
            " while removing its now-dead legacy branch (Issue #757 Step 2). Legacy"
            " had been the only path exercised by tests/test_fan_control.py and"
            " tests/test_fan_rate_limit.py (neither set _fan_fsm_authoritative, so"
            " both defaulted to legacy=False) — removing it forced every test through"
            " the FSM path, which has actually been running in real production for"
            " weeks, for the first time."
        ),
        "scope_covered": (
            "automation.py fan_thermostat_check(): the dispatch call to"
            " _resolve_fan_fsm_state(kind=THERMOSTAT_CHECK_TICK, ...) never passed"
            " indoor/outdoor/hysteresis/in_sleep_window overrides — confirmed via git"
            " show against the pre-#757 commit that this was already true before"
            " Step 2 touched the file, not a regression it introduced. _build_fan_fsm_"
            "inputs() defaults indoor/outdoor to None when not overridden, and"
            " decide_fan_thermostat_check()'s is_outdoor_rise_exit() short-circuits"
            " False on None, with Check 2 also requiring indoor is not None — so the"
            " function returned FanThermostatOutcome.KEEP unconditionally, every"
            " call, since the FSM branch went authoritative. Fixed by passing the"
            " same values already computed for the (still-used) FanThermostatInputs"
            " object built just above it, as explicit keyword overrides."
            " _build_fan_fsm_inputs(): added an optional fan_drift_tick_count"
            " override (matching the existing indoor/outdoor/hysteresis override"
            " convention) — _reconcile_fan_physical_drift() mutates"
            " self._fan_drift_tick_count to the real next_tick_count BEFORE"
            " dispatching DRIFT_TICK for mirror-sync, so fan_fsm.py's"
            " _transition_on_drift_tick() (which independently re-calls"
            " decide_fan_drift_reconciliation()) was reading the already-advanced"
            " count and silently re-progressing a second time, confirming drift one"
            " real tick early. The DRIFT_TICK dispatch call site now passes the"
            " pre-increment count explicitly (captured in the local FanDriftInputs"
            " built just above), so the FSM's re-derivation reaches the same outcome"
            " the real code just did instead of advancing past it."
            ' _activate_fan()/_deactivate_fan(): restored the INFO ("Fan toggle'
            ' deferred: rate limit...") and DEBUG ("...skipping duplicate report")'
            " log lines the deleted legacy closure _fan_toggle_rate_limited() used to"
            " own — same text/level split, now emitted directly in the FSM-only"
            " shell at both rate-limit dispatch sites."
            " tools/sim_harness/fan_thermostat_decision_integration.py: break_fan_"
            "thermostat_decision() patched"
            " custom_components.climate_advisor.automation.decide_fan_thermostat_"
            "check, which stopped affecting real behavior the moment Step 2 removed"
            " automation.py's own direct call to the pure function (fan_fsm.py has"
            " always had its own separate top-level import and call) — repointed to"
            " patch fan_fsm.decide_fan_thermostat_check instead, restoring the"
            " positive control's ability to detect a real regression."
        ),
    },
    762: {
        "version_fixed": "0.6.69",
        "title": (
            "3 latent design gaps in the door/window FSM-authoritative path, found"
            " while removing its now-dead legacy branch (Issue #757 Step 4). Unlike"
            " Issue #759's fan bugs, none had confirmed live-production impact —"
            " these were edge-case gaps invisible until removing the legacy flag-gate"
            " forced every test through the FSM path for the first time."
        ),
        "scope_covered": (
            "automation.py/nat_vent_fsm.py: _build_door_window_fsm_inputs() and"
            " _build_nat_vent_fsm_inputs() both defaulted an unconfigured fan_mode to"
            ' "whole_house_fan" instead of FAN_MODE_DISABLED, the convention every'
            " other of 25+ fan_mode reads in automation.py uses — could silently let"
            " a home with no whole-house fan configured skip a door/window pause it"
            " should have taken. Fixed by correcting both defaults."
            " door_window_fsm.py: SENSOR_OPENED's transition independently re-derives"
            ' "would nat-vent activate"/"is this a planned window" from raw'
            " readings — correct for handle_door_window_open()'s real fresh-open"
            " scenario, but wrong for _pause_for_door_window()'s two other real"
            " callers (_apply_comfort_band()'s choke-point guard,"
            " _sync_paused_by_door_with_live_sensors()), which already independently"
            " rule both out themselves before calling it, for reasons this FSM's"
            " simple re-derivation can't see. Confirmed live via golden scenario"
            " issue_629_comfort_band_arm_through_open_window: a windows-recommended"
            " day with the WHF manually turned off while readings stayed"
            " nat-vent-favorable landed on DoorOpenResponse.PLANNED_WINDOW_NOOP,"
            " silently leaving _paused_by_door=False even though HVAC was correctly"
            " being suppressed — decide_scheduled_band_gate()'s DEFER_PAUSED never"
            " engaged, a dashboard/status-consistency gap. Fixed via a new"
            " DoorWindowFsmInputs.nat_vent_gate_ruled_out field (default False,"
            " unaffected at handle_door_window_open()'s own dispatch), forcing both"
            " re-derived conditions False when set True at the two real"
            " _pause_for_door_window() call sites."
            " automation.py _re_pause_for_open_sensor(): the non-nat-vent-authoritative"
            " branch (nat-vent itself is not yet graduated — Step 5) was restored to"
            " directly recompute reactivation via _nat_vent_may_reactivate() rather"
            " than reading _paused_by_door, which is stale from a"
            " PAUSED_DURING_GRACE origin (_transition_from_paused_during_grace()'s"
            " GRACE_TIMER_EXPIRED handling never clears that flag regardless of the"
            " nested nat-vent outcome, by design) — a stale True there would mask a"
            " genuine nat-vent reactivation. Caught by"
