...
| `"running (untracked)"` | Thermostat reports fan running but CA's `_fan_active=False` — typical after HA restart or user-initiated fan run. Since Issue #510, also returned when a stale `_natural_vent_active` flag disagrees with confirmed-running physical state (see above). |
