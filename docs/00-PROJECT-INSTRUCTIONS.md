...
| What are all the known ways a user's manual thermostat change can be silently ignored or reversed by CA? | 7 bypasses catalogued; 3 fixed: setpoint-only not detected (#197 — fixed v0.3.55), bedtime/wakeup unconditional clear (#204 — fixed), grace timer lost on restart (#199/#227 — fixed v0.3.56). Open: override confirm lost on restart (#198), PATH B self-resolve (#200), second override ignored (#201), 30s guard too wide (#202). | [Debugging Guide — Override Bypass Inventory](09-DEBUGGING-GUIDE.md#override-bypass-inventory) |
