...
**Filter-then-limit order (Issue #432):** This sub-section, `build_event_timeline_table` (the Activity dashboard's timeline table — see [Activity Timeline Spec](activity-report-table.md)), and `build_override_details_context`'s fan-ownership scan all call the shared `filter_events_by_window(event_log, hours, now, *, limit=None)` helper (`ai_skills_context.py`) as the single source of truth for this filtering, replacing three previously-independent inline `event_log[-200:]` slices. The helper filters by the requested time window **first**, then applies the 200-entry render/context budget — the reverse of the old order, which could slice away in-window events before the time filter ever ran on a non-chronological log. It returns `(filtered_entries, limited)`: `limited=True` means the 200-entry budget trimmed an otherwise-larger in-window result. This is a distinct condition from the stored log simply not reaching back far enough to cover the requested window in the first place — that condition is surfaced at the API layer as `is_truncated`/`oldest_available` (see [REST API Brief](rest-api.md)), not by this provider.

**`TIMING CORRELATIONS` and `KNOWN OVERRIDE FALSE POSITIVES` now apply their own `filter_events_by_window` call (Issue #840).** `_build_timing_correlations(events, hours, now)` and `_build_known_override_false_positives(events, hours, now)` both take the raw `coordinator._event_log` and internally call `filter_events_by_window(events, hours, now)` (time window only, no `limit=` — unlike the 200-entry-capped call in the `EVENT LOG` sub-section above) before scanning. Before Issue #432 and through Issue #840, both functions received the raw, unfiltered `coordinator._event_log` with no time window and no size cap at all; that gap is what #840 closed. Issue #840 also replaced each function's nested O(events²) scan with a sort-once + `bisect` lookup per event. `EVENT_LOG_CAP` is 4000 (up from 500), so both changes matter for a long-lived log: the previous unbounded scan over 4000 events was both slower and could surface correlations far outside the requested window (Issue #864 tracks adding debug-level count/duration logging for future monitoring of these two providers).
