...
| What are the nat-vent session's 4 states, and is there a computed way to check the current one? | `INACTIVE`, `ACTIVE_FULL_GATE`, `ACTIVE_SOFT_START`, `PAUSED_REACTIVATION_LOCKOUT` — derived by `AutomationEngine.nat_vent_lifecycle_state`, differentially validated against every golden/pending scenario. | [Nat-Vent Lifecycle Spec](nat-vent-lifecycle-spec.md) |
