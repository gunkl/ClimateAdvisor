...
| What are the nat-vent session's 4 states, and is there a computed way to check the current one? | Four states: `INACTIVE`, `ACTIVE_FULL_GATE`, `ACTIVE_SOFT_START`, `PAUSED_REACTIVATION_LOCKOUT`. The FSM is now the sole authoritative implementation (the legacy branch and shadow engine were removed in Phase 6 graduation, Issue #757). | [FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer) |
| What are the door/window pause/grace lifecycle's states, and is anything actually driven by them? | Five states: `NORMAL`, `PAUSED_ACTIVE`, `PAUSED_IDLE`, `GRACE`, `PAUSED_DURING_GRACE`. The FSM is now the sole authoritative implementation (the legacy branch and shadow engine were removed in Phase 6 graduation, Issue #757). | [FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer) |
| Is there a named state for the override/grace joint lifecycle? | Composite state: `(OverrideConfirmState, GraceState)` pair — `OverrideConfirmState` is `IDLE`/`PENDING`; `GraceState` is `NONE`/`ACTIVE_PROTECTING_OVERRIDE`/`ACTIVE_UNPROTECTED`. The FSM is now the sole implementation (the legacy branch was removed in Phase 6 graduation, Issue #757). | [FSM Decision Layer](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer) |
| Is `lifecycle_dispatcher.py`'s cross-FSM event routing actually wired into production? | Yes (Issue #717) — wired into `AutomationEngine`, which both emits and consumes its own lifecycle events. | [FSM Decision Layer § Dispatcher Wiring](02-ARCHITECTURE-REFERENCE.md#fsm-decision-layer) |
| What is the fan/WHF lifecycle's composed state, and how is its FSM wired in? | `FanLifecycleState` with 5 independent axes (physical, override, cycling, HVAC ownership, rate-limit). The FSM is wired inside `AutomationEngine` and is now the sole implementation (the legacy branch was removed in Phase 6 graduation, Issue #757). | [Fan Lifecycle Spec](fan-lifecycle-spec.md) |
| Why does classification have no named lifecycle state, and what does its FSM actually compose? | Deliberately stateless — only a per-cycle decision with no multi-tick session. Composes `decide_scheduled_band_gate()` with `decide_ode_ceiling_guard()`. Now the sole implementation (the legacy branch was removed in Phase 6 graduation, Issue #757). | [Classification Lifecycle Spec](classification-lifecycle-spec.md) |
| What are the economizer's 3 states, and why does it have real session state unlike classification/occupancy? | Three states: `INACTIVE`, `COOL_DOWN`, `MAINTAIN`. The economizer persists a multi-tick session (unlike stateless classification/occupancy). Now the sole implementation (the legacy branch was removed in Phase 6 graduation, Issue #757). | [Economizer Lifecycle Spec](economizer-lifecycle-spec.md) |
...
| What are all the REST API endpoints and how does auth and data access work? | 21 views, HA long-lived token required. GETs read `coordinator.data`; POSTs delegate to coordinator or automation engine. Two config fields redacted. | [REST API Brief](rest-api.md) |
...
| How does window close/reopen timing get computed and displayed — what invariant must never break? | The ODE outdoor/indoor crossing scan in `compute_nat_vent_plan()` (`nat_vent_plan.py`); close/open pairs are always resolved together via `resolve_window_pair()`, which enforces open-strictly-after-close on the real dated datetimes (not bare clock times) when both sides are dynamic, and `nat_vent_cutoff_already_reached` flags a forward-only scan finding "now" rather than a genuine future crossing. 14+ prior incidents catalogued. | [Computation Reference §7a](08-COMPUTATION-REFERENCE.md#7a-window-timing-invariants--read-before-touching-this-feature-issue-878) |
...
The integration is at **v0.7.x** (currently v0.7.48) — 71 source modules in `custom_components/climate_advisor/` (verified via `ls custom_components/climate_advisor/*.py`, 2026-09-18; re-verify with that command rather than trusting this number long-term, since it drifts every time a module is added). Multi-zone support (Issue #796) shipped in v0.7.0 (2026-09-01) and has had 48 patch releases since. For the full module list with responsibilities and data-flow diagram, see the [Architecture Reference](02-ARCHITECTURE-REFERENCE.md). For design rationale and guiding principles, see [Strategy and Design](01-STRATEGY-AND-DESIGN.md).
