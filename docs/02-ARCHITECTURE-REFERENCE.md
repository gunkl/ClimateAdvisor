...
|---|---|---|
| What are the source files and what does each own? | 59 files total (2026-08-21). The original 16 own one responsibility each: `coordinator.py` orchestrates, `classifier.py` classifies, `automation.py` executes HVAC calls, `learning.py` persists and analyses behavior, etc. Block 5's FSM migration (epic #594) added 43 more — mostly small pure `decide_*()` decision-core modules plus the three lifecycle FSMs and the dispatcher pair. | [§File Structure](02-ARCHITECTURE-REFERENCE.md#file-structure) |
