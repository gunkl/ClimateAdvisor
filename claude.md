...
**Exception to "Next Automation... Must NOT contain: Time-of-day phrasing" (Issue #534, ratified #847-followup)**: a candidate may embed a time-disambiguation qualifier only when omitting it would read as a present-tense claim about something already true (not merely "to be thorough"). This is a bounded, registered carve-out for one candidate (`nat_vent_cutoff`) — not a general loosening of the no-duplication rule, and not an invitation to add more without clearing the same justification bar. Each such exception must be registered in `coordinator.py`'s `_ONTOLOGY_TIME_EXCEPTIONS` set with a one-line justification comment, and is enforced structurally, not by convention — see `tests/test_status_card_ontology.py`.
