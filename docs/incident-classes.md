...
|---|---|---|
| Which incident classes are proactive (fire at command time)? | Two: `setpoint_mode_inconsistency` — fires inside automation.py _set_temperature() before the HA service call; `hvac_command_failed` (Issue #805) — fires in automation.py _set_hvac_mode()/_set_temperature() when the HA service call itself raises (e.g. a removed climate_entity). Most other classes are reactive and fire post-cycle in coordinator._detect_and_emit_incidents. | [Proactive vs. Reactive column below](#incident-class-reference) |
| What is the `incident_detected` event payload schema? | `type`, `time`, `incident_class`, `incident_id`, `indoor_f`, `outdoor_f`, `hvac_mode`, `comfort_cool`, `comfort_heat`, plus class-specific fields (`nat_vent_active`, `manual_override_active`, `occupancy_mode`, `setpoint_f`, and — for `hvac_command_failed` (#805) — `climate_entity`, `error`). | [Event Payload](#event-payload) |
...
| `hvac_command_failed` | `hass.services.async_call("climate", ...)` raises inside `_set_hvac_mode()` or `_set_temperature()` — the two single write points (Fix 1b) for every thermostat command | **Proactive** | Integration | **HIGH** | #805 -- a removed/invalid `climate_entity` previously propagated uncaught, risking an aborted update cycle instead of a caught, logged, notified failure |
