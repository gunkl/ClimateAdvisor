...
**Issue #563 fix:** `scope_not_covered` removed from the schema entirely. `_select_relevant_fixes()` replaced the filter: always includes not-yet-deployed entries (`version_fixed > current`) plus the `_KNOWN_FIXES_RECENT_COUNT` (15) most recently fixed entries by count. Each entry rendered as the matching `RELEASE_NOTES[version_fixed]` bullet instead of the internal `title`/`scope_covered` engineering prose.

**Issue #702 update:** `RELEASE_NOTES`/`KNOWN_FIXES` no longer live in `const.py` — they made up 91% of that file (9,011 of 10,310 lines), imported and held resident in memory on every installation regardless of whether the (opt-in) AI Investigator was ever used. The data now lives in `fix_history.jsonl` (one JSON record per line, `issue`/`version_fixed`/`title`/`scope_covered`/`user_summary`), read via `fix_history.py`'s `search_records()` — a fresh streaming scan per call, nothing held resident between calls. `_select_relevant_fixes()`/`_release_note_bullet()` were removed; `build_known_fixes_context()` now calls `async_search_fix_history(hass, query=kwargs.get("focus", ""), limit=_KNOWN_FIXES_RECENT_COUNT, current_version=...)`, which ranks by relevance to the investigation's `focus` (when given) instead of recency alone — a fix from three releases ago that actually matches the symptom now outranks an unrelated fix from last release, which the old count-only window couldn't do. Not-yet-deployed entries are still always included. Each entry renders its `user_summary` when present, falling back to `title` — not every historical entry has a `user_summary` (some are internal/process changes with no occupant-facing outcome; see `tools/add_fix_entry.py`). The migration also consolidated the multi-release "strangler-fig FSM migration" effort (8 issue numbers, ~14KB of near-duplicate internal-refactor narrative) into a single record, and aggressively trimmed pre-0.5.x entries to issue/version/one-line title — see the fix_history record for issue #731's `merged_from` field for the full consolidated list. `tools/fix_search.py` exposes the same search standalone for developers, without going through the Investigator.
