...
**`TIMING CORRELATIONS` and `KNOWN OVERRIDE FALSE POSITIVES` do NOT use `filter_events_by_window`.** `_build_timing_correlations()` and `_build_known_override_false_positives()` both receive the raw, unfiltered `coordinator._event_log` with no time window and no 200-entry cap — this was true before Issue #432 and remains true after it; that fix only touched the three sites named above. Tracked as a follow-up (Issue #840): both scans are O(events²) in the worst case and their output has no size cap of its own, which could grow unreviewed now that `EVENT_LOG_CAP` is 4000 instead of 500.
