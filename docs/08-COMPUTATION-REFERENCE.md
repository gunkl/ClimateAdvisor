...
- **Family-switch lockout** (`comfort_mode_switch_min_interval_s`, default 600s, advanced config): once escalated, a switch back requires this minimum dwell since the family was last reaffirmed — armed on every heat/cool command and every nat-vent activate/deactivate (`_arm_comfort_family()`). Logged as `comfort_family_switch_locked_out` when a genuine escalation is blocked.
